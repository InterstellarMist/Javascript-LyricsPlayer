...
// Obtain access token
(async () => {
  local_token = await electronAPI.auth();
  console.log(local_token);
  ws.onopen = () => {
    ws.send(JSON.stringify({ local_token: local_token, action: "stream" }));
    ws.send(JSON.stringify({ local_token: local_token, action: "account" }));
  };
})();
...
ws.onmessage = (event) => {
  ...
  if (button_update && currentSongDetails.is_playing) {
    // console.log("played");
    pause_btn.src = "./assets/Pause Button.svg";
  } else if (button_update && !currentSongDetails.is_playing) {
    // console.log("paused");
    pause_btn.src = "./assets/Play Button.svg";
  }
};
...
function updatePlayer(data) {
  switch (Object.keys(data)[0]) {
    ...
    case "song":
      ...
      let position =
        (Number(song.progress_ms) / Number(song.duration_ms)) *
        empty_bar.offsetWidth;
      return position;
      break;
    case "lyrics":
      lyricPayload = data.lyrics;
      updateLyrics(data.lyrics);
    ...
    case "update":
      ...
      song_progress =
        Number(update.progress_ms) / Number(currentSongDetails.duration_ms);
...
function updateLyrics(lyricPayload) {
  ...
  for (let language of Object.keys(lyricPayload)) {
    lyrics.innerText = lyricPayload[language];
    switch (language) {
      case "English":
  ...
  if (lyrics_card_type) {
    if (Object.keys(lyricPayload).includes(lyrics_card_type)) {
      lyrics.innerHTML = lyricPayload[lyrics_card_type];
    } else {
      lyrics.innerHTML = Object.values(lyricPayload)[0];
    }
  }
}
...
pause_btn.addEventListener("click", () => {
  if (pause_btn.src.split(".")[0].toLowerCase().includes("pause")) {
    ws.send(JSON.stringify({ local_token: local_token, action: "pause" }));
    forceButtonPress(() => {
      pause_btn.src = "./assets/Play Button.svg";
      // console.log("working");
    });
  } else {
    ws.send(JSON.stringify({ local_token: local_token, action: "play" }));
    forceButtonPress(() => {
      pause_btn.src = "./assets/Pause Button.svg";
      // console.log("working");
    });
...
next_btn.addEventListener("click", () =>
  ws.send(JSON.stringify({ local_token: local_token, action: "next" }))
);
prev_btn.addEventListener("click", () =>
  ws.send(JSON.stringify({ local_token: local_token, action: "prev" }))
);
...
korean.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["Korean"];
  openLyricsCard();
  lyrics_card_type = "Korean";
});
english.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["English"];
  openLyricsCard();
  lyrics_card_type = "English";
});
chinese.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["Chinese"];
  openLyricsCard();
  lyrics_card_type = "Chinese";
});
japanese.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["Japanese"];
  openLyricsCard();
  lyrics_card_type = "Japanese";
});
romanized.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["Romanization"];
  openLyricsCard();
  lyrics_card_type = "Romanization";
});
original.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["Original"];
  openLyricsCard();
  lyrics_card_type = "Original";
});
french.addEventListener("click", () => {
  lyrics.innerHTML = lyricPayload["French"];
  openLyricsCard();
...
function openLyricsCard() {
  ...
  animate(
    0,
    (progress) => {
      move(menu_btn, menu_btn_prop, progress);
    },
    300
  );
  animate(
    0,
    (progress) => {
      move(profile_picture, profile_picture_prop, progress);
    },
    300
  );
  if (!lyrics_card_type) {
...
lyrics_pull_down.addEventListener("mousedown", () => {
  isDown = true;
});
